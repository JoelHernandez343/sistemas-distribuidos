...
cd files
zip -qr Servicio.zip Servicio/
cd ..
sshpass -p "${pw}" ssh "${USERNAME}@${IP}" "mkdir -p server"
sshpass -p "${pw}" scp files/sudo.sql files/deploy.sh files/root.sql files/usuario_sin_foto.png files/db.sql files/Servicio.zip "${USERNAME}@${IP}:~/server"
sshpass -p "${pw}" ssh "${USERNAME}@${IP}" "chmod +x server/deploy.sh"
rm files/Servicio.zip
stop_spinner $? '✅'